...
public class CutSceneManager : MonoBehaviour {
	...
	void  Start (){
		...
		if(GameObject.Find("Lif")==playerObject){
			...
			lifthrasirObject = GameObject.FindGameObjectWithTag("Lif");
		}
		Debug.Log ("Female: "+characterFemale);
		if (characterFemale == false) {
			...
			cutScenes.Add(new CutSceneObject(lifthrasirObject.transform, "Lif: Look, there’s Loki with Lifthrasir! I have to stop him and save her!"));
			cutScenes.Add(new CutSceneObject(lokiObject.transform, "Lif: What is Loki doing now??"));
			...
			cutScenes.Add(new CutSceneObject(playerObject.transform, "Lif: What does the rooster have to do with anything? I need to save Lifthrasir!"));
			...
			cutScenes.Add(new CutSceneObject(playerObject.transform, "Lif: Well, what are we waiting for? Let’s stop that rooster!"));
			...
			cutScenes.Add(new CutSceneObject(ravenObject.transform, "Odin: Wait, Lifthrasir. You must listen to me. We HAVE to stop that rooster. Surt is asleep right now. Loki has just convinced the rooster to wake him up and warn him that Ragnarok has begun. If that rooster gets to Surt and wakes him up, there is impending doom for all of us. He will use his sword to burn your home down!"));
			cutScenes.Add(new CutSceneObject(playerObject.transform, "Lifthrasir: Well, what are we waiting for? Let’s stop that rooster!"));
			cutScenes.Add(new CutSceneObject(roosterObject.transform, "Raven: Okay. Be careful not to let it make too much noise. The bar at the top right shows the noise meter. If that meter reaches the top and stays there, then it’s all over."));
			cutScenes.Add(new CutSceneObject(playerObject.transform, ""));
