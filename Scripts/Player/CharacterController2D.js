...
private var _characterState : CharacterState2D;
...
public var onPlatform = false;

private var groundedOn : GameObject;
...
function UpdateSmoothedMovementDirection ()
{
	...
	if (grounded)
	{
		...
		_characterState = CharacterState2D.Idle;
		
		// Pick speed modifier
		if (Input.GetKey (KeyCode.LeftShift) || Input.GetKey (KeyCode.RightShift))
		{
			targetSpeed *= runSpeed;
			_characterState = CharacterState2D.Running;
		}
		else if (Time.time - trotAfterSeconds > walkTimeStart)
		{
			targetSpeed *= trotSpeed;
			_characterState = CharacterState2D.Trotting;
		}
		else
		{
			targetSpeed *= walkSpeed;
			_characterState = CharacterState2D.Walking;
		}
...
function DidJump ()
{
	...
	_characterState = CharacterState2D.Jumping;
}
...
function Update() {
	...
	/*collisionFlags = */controller.MovePosition(movement);
	Debug.Log("Move Direction: "+moveDirection+".  Move Speed: "+moveSpeed+".  Vertical Speed: "+verticalSpeed+".  In Air Velocity: "+inAirVelocity+".  Velocity: "+controller.velocity+".  Is Grounded: "+IsGrounded());
	
	// ANIMATION sector
	if(_animation) {
		if(_characterState == CharacterState2D.Jumping) 
		{
		...
		else 
		{
			...
			{
				if(_characterState == CharacterState2D.Running) {
					_animation[runAnimation.name].speed = Mathf.Clamp(controller.velocity.magnitude, 0.0, runMaxAnimationSpeed);
					_animation.CrossFade(runAnimation.name);	
				}
				else if(_characterState == CharacterState2D.Trotting) {
					_animation[walkAnimation.name].speed = Mathf.Clamp(controller.velocity.magnitude, 0.0, trotMaxAnimationSpeed);
					_animation.CrossFade(walkAnimation.name);	
				}
				else if(_characterState == CharacterState2D.Walking) {
...
function Update() {
	...
	movement *= Time.deltaTime;
	var rigidbody : Rigidbody2D = GetComponent(Rigidbody2D);
...
function moveHorizontal() {
	var newDirection : Vector2 = this.transform.position;
	if(Input.GetKey("right")&&!Input.GetKey("left")){
		Debug.Log("Right");
		newDirection.x = newDirection.x+0.01;
